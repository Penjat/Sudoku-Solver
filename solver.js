...
function solve(){
  //is called to start of the solving process
  ...
  puzzleArray = new Array();
    ...
    solveData = {numLoops:0,solving:true,pos:{x:0,y:0},isGoingBack:false};



    //-------------------------------------------
    console.log("Solving...");
    solveLoop = setInterval(solveThread, solveSpeed);

}
function solveThread(){
  //the process that solves the puzzle
  //it can be called at varrying frequencies so the user can observe the process
  if(!solveData.solving){
    return;
  }


  solveData.numLoops++;
  console.log("looping" +solveData.numLoops );
  //return;
  var curCell = puzzleArray[solveData.pos.x][solveData.pos.y];

  //if it is a real value
  if(curCell.isReal){
    //if it is going back
    if(solveData.isGoingBack){
      //keep going back


      solveData.pos = goBack(solveData.pos);
      //check if back at begining
      if(checkBeg(solveData.pos)){
        solveData.solving = false;
        stopSolve();
        //puzzleSolved(puzzleArray);
      }
        //if it is going forward
        ...
        //keep going forward
        solveData.pos = addPos(solveData.pos);
        //check if we are at the end
        solveData.solving = checkEnd(solveData.pos);
      }
    //if it is not a real value
    }else{
      //check the posibilities
      curCell.curNum = checkPosNums(solveData.pos,curCell.curNum);
      //if no posibilities were found
      if(curCell.curNum == 0){
        //Go back
        var x = solveData.pos.x;
        var y = solveData.pos.y;
        var cellName = "cell:"+x+":"+y;
        document.getElementById(cellName).innerHTML = " ";
        solveData.pos = goBack(solveData.pos);
        solveData.isGoingBack = true;
        //check if we are at the begining
        if(checkBeg(solveData.pos)){
          solveData.solving = false;
          stopSolve();
      ...
      //if a number was found
        ...
        var y = solveData.pos.y;
        var cellName = "cell:"+x+":"+y;
        document.getElementById(cellName).innerHTML = puzzleArray[x][y].curNum;
        //adds the position, returns false when we reach the end
        solveData.isGoingBack = false;
        solveData.pos = addPos(solveData.pos);
        solveData.solving = checkEnd(solveData.pos);
      }
    }
...
function setSolveSpeed(speed,but){
  if(speedBut){
    speedBut.style.background='#f2f2f2';
  }
  speedBut = but;
  speedBut.style.background='#4d94ff';
  solveSpeed = speed;
}
